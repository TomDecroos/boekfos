...
class FloorPlan(Collection):
    ...
    def draw(self, figsize=1):
        ...
        plt.figure(figsize=(figsize * (x2 - x1) * 1.1, figsize * (y2 - y1)))
        ax = plt.gca()
        super().draw(ax)
...
class Wall(Collection):
    def __init__(self, a, b1, b2, orientation, alignment):
        super().__init__()
        r = ComplexRect(a, b1, b2, orientation, self.width, self.color,
                        alignment)
        self.add(r)
    ...
    def window(self, b1, b2):
        for item in self.items:
            if item.b1 < b1 and item.b2 > b2:
                ...
                itcopy.b1 = b2
                item.b2 = b1
                window = ComplexRect(item.middle_a(), b1, b2, item.orientation,
    ...
    def door(self, b, alignment1, alignment2, doorwidth=0.85):
        ...
        for item in self.items:
            if isinstance(item, ComplexRect) and item.b1 < b1 and item.b2 > b2:
                ...
                door = Door(x, y, degrees2, degrees1, doorwidth)
...
class Door:
    ...
    def draw(self, ax):
        ...
        ax.add_patch(
            Wedge(
                (self.x, self.y), self.deurwidth, d1, d2, fill=False, ls=":"))


def get_degrees(alignment):
    for name, degrees in [("top", 90), ("bottom", 270), ("left", 180),
                          ("right", 360)]:
        if alignment == name:
            return degrees
...
def get_multiplier(alignment):
    for name, m in [("top", 1), ("bottom", -1), ("left", -1), ("right", 1)]:
        if alignment == name:
            return m
